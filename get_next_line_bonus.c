...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/12/19 10:44:06 by aelbouab          #+#    #+#             */
/*   Updated: 2023/12/20 15:27:17 by aelbouab         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (!ft_strchr(rst[fd], '\n'))
	{
		free (rst[fd]);
		rst[fd] = NULL;
		rst[fd] = redbuf(rst[fd], fd);
		if (!rst[fd])
			return (free(line), NULL);
		line = freeline(line, rst[fd]);
	}
	rst[fd] = cleanbuf(rst[fd]);
	if (!line || *line == '\0')
		return (free (line), free(rst[fd]), NULL);
	return (line);
}
